...
const OrderBookingSheet = ({ fetchData }) => {
    ...
    useEffect(() => {
        async function fetchSalesOrderDetails() {
            if (source === "dropdown" && selectedSalesOrderName) {
                try {
                    const res = await axios.post(`${import.meta.env.VITE_CS365_URI}/api/orders/order-booking`, { name: selectedSalesOrderName });
                    const data = res.data;
    ...
    async function onSubmit(values) {
        try {
            await axios.post(`${import.meta.env.VITE_CS365_URI}/api/orders/save-order`, {
                ...values,
...
useEffect(() => {
  ...
  form.reset({
    ...
    salesOrderValue: selectedSalesOrder.salesOrderValue || 0,
    company: selectedSalesOrder.company || "",
    adjustment: selectedSalesOrder.adjustment || 0,
                                                        ...
                                                        <Input
                                                            ...
                                                            onChange={e => {
                                                                ...
                                                                if (val === '' || val === '-' || /^-?\d*(\.\d*)?$/.test(val)) {
                                                                    ...
                                                                    const salesOrderValue = form.getValues("salesOrderValue") || 0;
                                                                    form.setValue("adjustedSalesValue", salesOrderValue + (isNaN(num) ? 0 : num));
                                                                }
                                                            }}
                                                        />
                                                    </FormControl>
